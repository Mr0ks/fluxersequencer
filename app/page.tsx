...
export default function Home() {
  ...
  const [audioSplitterOpen, setAudioSplitterOpen] = useState(false);
  ...
  function onEventPointerDown(pointer: ReactPointerEvent<HTMLElement>, cue: CueEvent) {
    ...
    const move = (event: PointerEvent) => {
      moved = true;
      const nextTime = clamp(snap(startTime + ((event.clientX - startX) / rect.width) * project.duration), 0, Math.max(0, project.duration - cue.duration));
  ...
  function onEventResizePointerDown(pointer: ReactPointerEvent<HTMLElement>, cue: CueEvent, edge: "left" | "right") {
    pointer.stopPropagation();
    setSelectedEventId(cue.id);
    setSelectedLaneId(cue.laneId);
    const article = pointer.currentTarget.parentElement as HTMLElement;
    const rect = article.parentElement!.getBoundingClientRect();
    const startX = pointer.clientX;
    const startTime = cue.time;
    const startDuration = cue.duration;
    const endTime = startTime + startDuration;
    const before = project;
    let moved = false;
    const move = (event: PointerEvent) => {
      moved = true;
      const delta = ((event.clientX - startX) / rect.width) * project.duration;
      let time = startTime;
      let duration = startDuration;
      if (edge === "left") {
        time = clamp(snap(startTime + delta), 0, endTime - 0.1);
        duration = endTime - time;
      } else {
        const nextEnd = clamp(snap(endTime + delta), startTime + 0.1, project.duration);
        duration = nextEnd - startTime;
      }
      time = Number(time.toFixed(3));
      duration = Number(Math.max(0.1, duration).toFixed(3));
      setProject((current) => ({
        ...current,
        events: current.events.map((item) => item.id === cue.id ? {
          ...item,
          time,
          duration,
          hold: Math.max(0, duration - item.fadeIn - item.fadeOut),
        } : item),
      }));
    };
    const up = () => {
  ...
  function onPlayheadPointerDown(pointer: ReactPointerEvent<HTMLElement>) {
    pointer.stopPropagation();
    const panel = pointer.currentTarget.parentElement?.parentElement as HTMLElement;
    const rect = panel.getBoundingClientRect();
    const timelineStart = rect.left + 145;
    const timelineWidth = Math.max(1, rect.width - 145);
    const scrub = (clientX: number) => seek(snap(((clientX - timelineStart) / timelineWidth) * project.duration));
    scrub(pointer.clientX);
  ...
  function downloadPlayer() {
    ...
    setToast("Physical Roblox board downloaded — import it and press Play");
  }
    ...
    <main className="sequencer-shell">
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          <button className="audio-splitter-button" onClick={() => setAudioSplitterOpen(true)}>✂ Audio splitter</button>
      ...
      <section className="workspace">
        ...
        <section className="timeline-panel">
              ...
              <div className={`lane ${selectedLaneId === lane.id ? "selected" : ""}`} key={lane.id}>
                ...
                <div className="lane-grid" onPointerDown={(event) => onLaneClick(event, lane.id)}>
                  {project.events.filter((cue) => cue.laneId === lane.id).map((cue) => (
                    <article key={cue.id} className={selectedEventId === cue.id ? "selected-event" : ""} style={{ left: `${(cue.time / project.duration) * 100}%`, width: `${Math.max(1.2, (cue.duration / project.duration) * 100)}%`, background: EVENT_COLORS[cue.type] || (cue.mode === "effect" ? "#b9a7ff" : "#96e8c5") }} onPointerDown={(event) => onEventPointerDown(event, cue)} onDoubleClick={() => duplicateEvent(cue)}>
                      <span className="resize-handle left" role="slider" tabIndex={0} aria-label={`Resize start of ${cue.label}`} aria-valuemin={0} aria-valuemax={cue.time + cue.duration - 0.1} aria-valuenow={cue.time} onPointerDown={(event) => onEventResizePointerDown(event, cue, "left")} onDoubleClick={(event) => event.stopPropagation()} />
                      <strong><i className={`layer-dot ${cue.layer}`} />{cue.label}</strong><small>{eventSummary(cue)} · {formatTime(cue.time, true)}</small>
                      <span className="resize-handle right" role="slider" tabIndex={0} aria-label={`Resize end of ${cue.label}`} aria-valuemin={cue.time + 0.1} aria-valuemax={project.duration} aria-valuenow={cue.time + cue.duration} onPointerDown={(event) => onEventResizePointerDown(event, cue, "right")} onDoubleClick={(event) => event.stopPropagation()} />
                    </article>
                  ))}
                </div>
              </div>
          ...
          <div className="playhead" style={{ left: `calc(145px + (100% - 145px) * ${currentTime / project.duration})` }}><span role="slider" aria-label="Move play marker" aria-valuemin={0} aria-valuemax={project.duration} aria-valuenow={currentTime} onPointerDown={onPlayheadPointerDown}>{playing ? "▶" : "II"}</span></div>
        </section>
      ...
      {audioSplitterOpen && <AudioSplitter onClose={() => setAudioSplitterOpen(false)} />}
